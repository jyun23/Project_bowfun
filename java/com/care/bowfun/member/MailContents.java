...
public class MailContents implements IMailService {
	...
	public MimeMessage creatMessage(String to) throws MessagingException, UnsupportedEncodingException {
		...
		message.setSubject("[BOWFUN] 회원가입을 위한 이메일 인증코드 입니다"); // 이메일 제목

		String msgg = "";
		// msgg += "<img src=images/bowfun.png/>"; // header
		// image
		msgg += "<h1>안녕하세요</h1>";
		msgg += "<h1>BOWFUN 입니다</h1>";
		msgg += "<br>";
		msgg += "<p>아래 인증코드를 입력해주세요</p>";
		msgg += "<br>";
		msgg += "<br>";
		msgg += "<div align='center' style='border:3px solid black'>";
		msgg += "<h3 style='color:gold'>회원가입 인증코드 입니다</h3>";
		...
		message.setFrom(new InternetAddress("qorthgml2002@naver.com", "BOWFUN"));
