...
//		if(file.getSize() != 0) { // 클라이언트가 파일을 업로드 했다면
	...
	public String NoticeModifyProc(MultipartHttpServletRequest multi, String no) {
		...
		Notice.setNo(Integer.parseInt(multi.getParameter("no")));
		Notice.setTitle(multi.getParameter("title"));
		Notice.setContent(multi.getParameter("content"));
		Notice.setFileName("");
		
		String deleteFullPath = Notice.getFileName();
		if (deleteFullPath != null) { // 테이블에 파일의 경로와 이름이 있다면
			File df = new File(deleteFullPath);
			if (df.exists() == true) // 파일 저장소에 파일이 존재한다면
				df.delete();
		}
